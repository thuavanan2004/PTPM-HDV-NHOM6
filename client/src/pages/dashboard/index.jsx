...
function Dashboard() {
  const [dashboardTours, setDashboardTours] = useState([]);
  const [dashboardOders, setDashboardOders] = useState([]);
  ...
  const monthlyStatsData = {
    labels:
      dashboardTours?.monthlyStats
        ?.map((stat) => `Tháng ${stat.month}/${stat.year}`)
        .reverse() || [],
    datasets: [
      {
        label: "Số lượng",
        data:
          dashboardTours?.monthlyStats?.map((stat) => stat.count).reverse() ||
          [],
        fill: false,
        borderColor: "#42A5F5",
  ...
  const weeklyStatsData = {
    labels:
      dashboardTours?.weeklyStats
        ?.map((stat) => `Tuần ${stat.week}/${stat.year}`)
        .reverse() || [],
    datasets: [
      {
        label: "Số lượng",
        data:
          dashboardTours?.weeklyStats?.map((stat) => stat.count).reverse() ||
          [],
        fill: false,
        borderColor: "#66BB6A",
  ...
  const orderStatusData = {
    ...
    datasets: [
      {
        data: [
          dashboardOders?.pendingOrders,
          dashboardOders?.confirmedOrders,
          dashboardOders?.canceledOrders,
        ],
  ...
  const weeklyRevenueData = {
    labels:
      dashboardOders?.revenueByWeek
        ?.map((item) => `Tuần ${item.week}/${item.year}`)
        .reverse() || [],
    datasets: [
      {
        label: "Doanh thu",
  ...
  const paymentMethodRevenueData = {
    labels:
      dashboardOders?.revenueByPaymentMethod
  ...
  const cancellationRateData = {
    labels:
      dashboardOders?.cancellationRate
        ?.map((item) => `Tháng ${item.month}/${item.year}`)
        .reverse() || [],
    datasets: [
      {
